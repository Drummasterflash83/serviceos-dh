# Decision & Operational Modes Engine

**Status:** Implemented and machine-verified (the pure engine is proven by
[decision.verify.ts](../../supabase/functions/_shared/intelligence/decision.verify.ts);
`decision_log` immutability is proven by `decision_log_immutable.test.sql`). The
schema migrations are additive and statically reviewed; deployment follows the
state indicated in the code.

This engine is **[Core Loop](../architecture/02_CORE_LOOP.md) stage 5
(Recommendation → Decision)** plus the autonomy gate that sits between the Decision
and execution. It answers two separate questions, and it never merges them:

> **The Decision Engine:** who or what owns the next move, and why?
>
> **The Operational Modes Engine:** how much is this business ready to let us act on
> its own?
...
This document explains what it is and the boundaries it must never cross. Read the
[Core Loop](../architecture/02_CORE_LOOP.md) and the
[canonical glossary](../architecture/00_GLOSSARY.md) for how these terms connect,
and [foundation/OPENFOLK_PHILOSOPHY.md](../foundation/OPENFOLK_PHILOSOPHY.md) for the
philosophy this engine mechanically expresses.

The **five Operational Modes** (`discovery`, `recommendation`, `assisted`,
`trusted`, `optimisation`) **are** the five customer-maturity stages defined in the
[Continuous Optimisation constitution](../foundation/OPENFOLK_CONTINUOUS_OPTIMISATION.md)
(Discovery, Recommendation, Assisted Automation, Trusted Automation, Continuous
Optimisation). They are one concept, named once here and once there.
...
- **The [Automation Engine](AUTOMATION_ENGINE.md) consumes the
...
- `objectiveContext` — which business objectives this decision relates to,
  **resolved before entry** and carried verbatim. It is descriptive only: the
  ...
  goal-seeking. See [OBJECTIVES_AND_OUTCOMES.md](OBJECTIVES_AND_OUTCOMES.md).
...
Both names are deliberate: `AUTOMATION_AUTHORISED` reads as if it executes and
`AUTOMATION_REQUIRES_APPROVAL` as if it only prepares, but **both merely authorise
and neither executes**. The canonical set of nine destinations, taken directly from
the `DecisionDestination` union and the routing precedence in
[decision.ts](../../supabase/functions/_shared/intelligence/decision.ts), is:
`AUTOMATION_AUTHORISED`, `AUTOMATION_REQUIRES_APPROVAL`, `OPENFOLK_REVIEW`,
`TENANT_SENIOR_REVIEW`, `CUSTOMER_APPROVAL`, `WAIT_FOR_EVENT`, `ESCALATE`, `REJECT`,
`NO_ACTION`. These strings are used identically across the type union, the
destination registry, routing, tests and this document.
