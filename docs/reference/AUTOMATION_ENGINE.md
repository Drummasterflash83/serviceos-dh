...
It never answers _"should the business do this?"_, that was settled by the
...
_current_ validity; it never re-runs policy. Outcomes:
...
used**, against the _current_ profile — so a tenant demoted from `trusted` to `discovery`
...
types `send_marketing_test_email`, `send_marketing_broadcast_email` (Phase 5)
AND `send_marketing_sequence_email` (Phase 6), capability `email.send_marketing`,
`external_side_effect = true`, risk `high`). The three frozen envelope shapes are
disjoint exact allowlists discriminated by `purpose`; the intent type must match
the envelope's declared purpose. A SEQUENCE send carries enrolment/revision/
step/execution lineage instead of a broadcast's snapshot/member/dispatch
lineage, and rechecks its own canonical authority
(`marketing_sequence_send_authority`) immediately before the single provider
call — same discipline, different binding. The BROADCAST intent type registers
...
| `supabase/migrations/20260903120000_marketing_sequences.sql`                    | Marketing Phase 6 registration: the APPROVAL-REQUIRED `send_marketing_sequence_email` intent type on the same capability (its own pre-provider authority `marketing_sequence_send_authority`), plus the NEW INTERNAL capability `marketing.contact_action` (`external_side_effect = false`) with five intent types — `marketing_apply_tag`, `marketing_remove_tag`, `marketing_change_lifecycle`, `marketing_assign_owner`, `marketing_create_follow_up` — each executed by the `marketing_actions` adapter through the EXISTING governed mutation RPCs. `marketing_create_follow_up` stays registered but is refused at authoring time (MK428) on any platform with no `('core','Action')` state machine, because the work item it would create could never be progressed; the gate reads `state_transitions` directly and opens with no Marketing change. The internal action adapter is convergent: a canonical mutation that already reached its target is reported as converged rather than retried, so a finalisation failure after a successful write cannot strand an execution |
| `supabase/functions/_shared/connectors/marketing_actions.ts`                    | the registered INTERNAL Marketing actions adapter: no network call, no direct table write; every change goes through a canonical governed RPC                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `supabase/migrations/20260904120000_marketing_templates_reporting_ai.sql`      | Marketing Phase 7 registration: the SECOND deliberate, reviewed external capability — `ai.generate_marketing_draft` (external, risk medium, `supports_status_lookup` false) with intent type `generate_marketing_draft` and the operational outcome `marketing_ai_draft_recorded`. `requires_approval` is FALSE **honestly**: a generation is an explicitly authorised, DELEGATED draft action under canonical `marketing.campaigns.draft` (the Phase-4 delegated-test-send precedent); NO approval row exists or is fabricated, the package records the call as irreversible external work so the Operational-Mode re-check withholds it in modes requiring reversibility, and the result can never send, approve or launch — its only write is one immutable proposal. Per-tenant enablement is `tenant_connector_capabilities` (`openai` / `ai.generate_marketing_draft`), flipped only by owner/admin provider configuration — never seeded |
| `supabase/functions/_shared/connectors/marketing_ai_draft.ts`                   | the registered AI draft adapter: exactly ONE provider call, credentials from the tenant Vault broker only (never a global environment key), canonical actor-authority + provider-configuration rechecks fail-closed pre-provider, output gated by the exact schema parser AND the canonical Marketing content validator, persistence ONLY through the governed `marketing_ai_record_proposal` RPC (idempotent per intent); provider refusals/truncation/malformed output are stable permanent results, 429/5xx/network are bounded transients ONLY before the provider answers; after a successful provider call a recorder/persistence failure retries the idempotent recorder RPC once in-place and otherwise returns `unknown` — the engine freezes automatic retry (`external_result_unknown`) so one logical generation can never re-buy the paid call (an explicit human "generate again" is a NEW request + intent). Held to this by conformance gate (k) and the pure-suite one-fetch invariant |
