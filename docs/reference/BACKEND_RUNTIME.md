...
| `marketing.sequence_advance` | `handleMarketingSequenceAdvance` | Bounded, lease-safe Sequence step runner (Phase 6): apply evidence-backed exits (suppression/reply/lifecycle), claim ≤10 due enrolment steps (FOR UPDATE SKIP LOCKED, lease recovery) — WAIT steps are resolved inside that SQL from the factual arrival time so a late worker can never make a wait drift — then per side-effecting step recheck the ONE canonical sequence authority, render deterministically from frozen inputs, create the governed engine lineage (approval-required email intent with a genuine tenant_senior approval, or an internal `marketing.contact_action` intent), enqueue execution, reconcile internal actions, and run the same orphaned-intent recovery sweep as Broadcasts. Continues only while genuinely claimable work remains. Never calls a provider. |
