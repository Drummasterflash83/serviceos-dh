...
`intelligence.observe` DecisionPackage pipeline. It is **channel-neutral, reusable, and
...
channel-neutral ObservationDraft
        ↓  intelligence.observe   (unchanged — the SOLE DecisionPackage path)
immutable DecisionPackage → existing review routing
```
...
- **`intelligence.observe` remains the sole DecisionPackage path.** The bridge only
  _enqueues_ `intelligence.observe`; it never calls the Decision Engine directly and
  never creates an Automation Intent or any external side effect.
...
- Steps 2–4: `review_resolve` for DecisionPackages, executable controlled intents,
