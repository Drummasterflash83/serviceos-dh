// ServiceOS — Edge Function: business-graph-scheduled-sync
//
// Keeps the Business Graph current automatically. On a cron (NO user session) it
// invokes `business-graph-sync` server-to-server for every operational tenant,
// projecting people/companies/cards/interactions/recommendations into the graph.
// Idempotent, failure-isolated per tenant. The identity engine also best-effort
// triggers a sync on enrichment; this scheduler is the reliable backstop. Manual
// "Build graph" is a recovery override.
//
// Auth: a shared secret header `x-schedule-secret` must match GRAPH_SYNC_SECRET,
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const expected = Deno.env.get("GRAPH_SYNC_SECRET");
  if (!expected) return fail("config_error", "GRAPH_SYNC_SECRET is not configured", 500);
  ...
  for (const tenantId of tenantIds) {
    ...
    results.push({
      ...
      nodes_upserted: typeof j?.nodes_upserted === "number" ? j.nodes_upserted : 0,
      edges_upserted: typeof j?.edges_upserted === "number" ? j.edges_upserted : 0,
