// ServiceOS — Edge Function: recommendation-scheduled-sync
//
// Keeps recommendations current automatically. On a cron (NO user session) it
// invokes `recommendation-sync` server-to-server for every operational tenant,
// re-applying the deterministic rules to customer-card projections. Runs AFTER
// customer-card sync (Graph → Customer Cards → Recommendations) — no dependency
// cycle. Idempotent, failure-isolated per tenant. Manual "Generate" is an
// override; normal operation is automatic.
//
// Auth: a shared secret header `x-schedule-secret` must match
// RECOMMENDATION_SYNC_SECRET, else 403 (same model as the other scheduled
// functions). No secrets logged.
//
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const expected = Deno.env.get("RECOMMENDATION_SYNC_SECRET");
  if (!expected) return fail("config_error", "RECOMMENDATION_SYNC_SECRET is not configured", 500);
  ...
  for (const tenantId of tenantIds) {
    const res = await invokeFunction("recommendation-sync", { tenant_id: tenantId }, serviceKey);
    const j = res.json;
    results.push({
      tenant_id: tenantId,
      ...
      closed: typeof j?.closed === "number" ? j.closed : 0,
