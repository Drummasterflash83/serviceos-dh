// ServiceOS — Edge Function: customer-card-sync (Customer Card Engine v1)
//
// Projects the Business Graph + its source facts (interactions, recommendations,
// people, companies) into a clean OPERATIONAL projection stored in
// `customer_cards.context.projection`. Customer cards are NOT a source of truth —
// they are a projection of the graph. This engine stores only CALCULATED
// summaries (identity / communication / operations / business / timeline), never
// duplicated source rows, and everything is explainable (scores carry inputs).
//
// Manually-confirmed fields (customer_cards.locked_fields) are NEVER overwritten.
//
// Auth: user session (owner/admin/ops) OR the internal service path (scheduler).
// Tenant bound server-side. No secrets logged; no transcript/audio in the card.
//
// Request body: { customer_card_id?: uuid, graph_node_id?: uuid, limit?: number }
// Returns: { success, cards_projected, skipped, failed, health_counts }
//
...
import {
  ...
  responsiveness,
  type Health,
} from "../_shared/customer_card.ts";

const DEFAULT_LIMIT = 200;
const MAX_LIMIT = 500;
// How many recent interactions per card feed the projection window.
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    return failResponse("card_sync_error", message, 500);
  }
});
