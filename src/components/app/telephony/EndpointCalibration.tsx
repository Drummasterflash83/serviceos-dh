/**
 * Reusable endpoint → staff calibration surface (STEP 10). Shows provider capabilities and
 * the discovered endpoint inventory with evidence-based suggestions; lets an admin confirm /
 * reject / mark-shared / assign any tenant person. Suggestions are never auto-applied — an
 * unmapped endpoint stays "Unknown team member". Used both inside the onboarding wizard and
 * on the connected-state settings surface.
 */

import { useCallback, useEffect, useState } from "react";
import { Loader2, Check, X, Users, AlertTriangle } from "lucide-react";
...
export const CAP_TONE: Record<CapabilityState, string> = {
  ...
  return (
    <div className="relative mt-1.5">
      <input
        value={q}
        disabled={disabled}
        aria-label="Assign another person"
        onChange={(e) => {
          setQ(e.target.value);
          setOpen(true);
        }}
...
/**
 * The calibration list. `onSummary` bubbles counts so a parent wizard can gate progress on
 * resolution coverage. `showCapabilities` toggles the capability grid (hidden when the parent
 * already shows it).
 */
export function EndpointCalibration({
  onChanged,
  onSummary,
  showCapabilities = true,
}: {
  onChanged?: () => void;
  onSummary?: (s: CalibrationSummary) => void;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const r = await getTelephonyInventory();
    setInv(r);
    setLoading(false);
    if (r.ok && onSummary) {
      const c = r.data.counts;
      const confirmedOrShared = r.data.endpoints.filter(
        (e) => e.mapping?.status === "confirmed" || e.mapping?.is_shared_device,
      ).length;
      onSummary({
        ...
        unresolved: Math.max(0, r.data.endpoints.length - confirmedOrShared),
      });
    }
  }, [onSummary]);
  useEffect(() => {
    ...
    <div className="space-y-3">
      ...
      {data && showCapabilities && (
        <div className="rounded-2xl border border-hairline bg-white p-5">
          ...
          <div className="mt-3">
            <CapabilitiesGrid capabilities={data.capabilities.capabilities} />
          </div>
        </div>
      )}
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    ...
                    <button
                      disabled={busy === e.endpoint_ref}
                      onClick={() => act(() => setEndpointUnknown(e.endpoint_ref), e.endpoint_ref)}
                      className="inline-flex items-center gap-1 rounded-full border border-hairline px-2.5 py-1 text-[11px] font-medium text-muted-foreground hover:bg-white disabled:opacity-50"
                    >
                      Unknown
                    </button>
